...
const extended_validator = require('./extended-validator')
...
class LinValidator {
  ...
  async validate(ctx, alias) {
    ...
    this.parsed = Object.assign(Object.assign({}, tmpData), {
      default: {}
    })
    if (!(await this.checkRules())) {
      ...
      throw new exception.ParameterException(obj)
    } else {
      ctx.v = this
      return this
    }
  ...
  /**
   *
   * @param {Array} fields 参数字段数组 ['field1', 'field2', ...]
   * @param {String} path body or path，从 body 取参数还是从 path 取参数
   * @param {Boolean} isFilterEmpty 是否过滤掉空参数
   */
  getAll(fields, path = 'body', isFilterEmpty = false) {
    const data = {}
    fields.forEach(field => {
      const value = this.get(`${path}.${field}`)
